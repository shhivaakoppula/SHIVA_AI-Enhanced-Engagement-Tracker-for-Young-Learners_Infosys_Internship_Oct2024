...
if not os.path.exists("Shiva_landmark_screenshots"):
    os.makedirs("Shiva_landmark_screenshots")
...
try:
    while True:
        ...
        for (top, right, bottom, left), face_encoding in zip(face_locations, face_encodings):
            ...
            if distance < 0.6:
                ...
                screenshot_filename = f"Shiva_landmark_screenshots/{name}_{now.strftime('%Y-%m-%d_%H-%M-%S')}.jpg"
                if attentive:
        ...
        if (datetime.now() - last_save_time) > timedelta(seconds=30):
            df.to_excel('Shiva_attendance_with_attention_landmark.xlsx', index=False)
            print("Data saved to 'Shiva_attendance_with_attention_landmark.xlsx'")
    ...
    if not df.empty:
        df.to_excel('Shiva_attendance_with_attention_landmark.xlsx', index=False)
        print("Final data saved to 'Shiva_attendance_with_attention_landmark.xlsx'")
