...
if not os.path.exists("Shiva_excel_sc_screenshots"):
    os.makedirs("Shiva_excel_sc_screenshots")

# Load the known image of Guru Teja
known_image = face_recognition.load_image_file("C:\\Users\\Shiva Goud\\Desktop\\INFOSYS SPRING BOARD\\Face_Recognition\\Shiva_Goud_Koppula\\Input Image\\shiva.png")
...
try:
    while True:
        ...
        for face_encoding in face_encodings:
            ...
            if distance < confidence_threshold:
                ...
                if last_recognition_time is None or (now - last_recognition_time) >= entry_duration:
                    # Save screenshot
                    screenshot_filename = f"Shiva_excel_sc_screenshots/Shiva_{now.strftime('%Y-%m-%d_%H-%M-%S')}.jpg"
                ...
                for face_location in face_locations:
                    ...
                    cv.putText(frame, 'Shiva', (left, top - 10), cv.FONT_HERSHEY_SIMPLEX, 0.5,
                               (255, 0, 0), 2, cv.LINE_AA)

        if not recognized:
            # If the face is not recognized, label it as 'Not Guru Teja'
            cv.putText(frame, 'Not Shiva', (30, 55), cv.FONT_HERSHEY_SIMPLEX, 1,
                       (255, 0, 0), 2, cv.LINE_AA)
        ...
        if (datetime.now() - last_save_time).total_seconds() >= 30:
            df.to_excel('Shiva_attendance_with_excel_sc_screenshots.xlsx', index=False)
            print("DataFrame saved to 'Shiva_attendance_with_excel_sc_screenshots.xlsx'")
    ...
    if not df.empty:
        df.to_excel('Shiva_attendance_with_screenshots(4).xlsx', index=False)
        print("Final attendance and screenshots saved to 'Shiva_attendance_with_screenshots(4).xlsx'.")
