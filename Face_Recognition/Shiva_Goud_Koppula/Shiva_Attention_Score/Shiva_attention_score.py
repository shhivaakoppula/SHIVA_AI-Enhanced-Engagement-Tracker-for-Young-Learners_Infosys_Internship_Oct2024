...
if not os.path.exists("Shiva_screenshots_attention_score"):
    os.makedirs("Shiva_screenshots_attention_score")
...
try:
    while True:
        ...
        for (top, right, bottom, left), face_encoding in zip(face_locations, face_encodings):
            ...
            if distance < 0.6:
                ...
                screenshot_filename = f"Shiva_screenshots(7)/{name}_{now.strftime('%Y-%m-%d_%H-%M-%S')}.jpeg"
                if attentive == 'Yes':
        ...
        if (datetime.now() - last_save_time).total_seconds() >= 30:
            df.to_excel('Shiva_attendance_with_attention_score.xlsx', index=False)
            print("DataFrame saved to 'Shiva_attendance_with_attention_score.xlsx'")
    ...
    if not df.empty:
        df.to_excel('Shiva_attendance_with_attention_score.xlsx', index=False)
        print("Final attendance with attentiveness saved to 'Shiva_attendance_with_attention_score.xlsx'.")
