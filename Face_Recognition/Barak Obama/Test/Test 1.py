...
known_image = face_recognition.load_image_file("C:\\Users\\Shiva Goud\\Desktop\\INFOSYS SPRING BOARD\\Face_Recognition\\Barak Obama\\Test\Barak Obama.jpg")
...
cam = cv.VideoCapture("C:\\Users\\Shiva Goud\\Desktop\\INFOSYS SPRING BOARD\\Face_Recognition\\Barak Obama\\Test\\1000097197.mp4")
if not cam.isOpened():
...
try:
    while True:
        ...
        if recognition_count >= 5:
            df.to_excel('Barak_obama_test_attendence.xlsx', index=False)
            recognition_count = 0
    ...
    if not df.empty:
        df.to_excel('Test_attendance.xlsx', index=False)
