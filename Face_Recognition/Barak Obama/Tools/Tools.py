...
known_image = face_recognition.load_image_file("C:\\Users\\Shiva Goud\\Desktop\\INFOSYS SPRING BOARD\\Face_Recognition\\Barak Obama\\Tools\\Barak Obama.jpg")
...
cam = cv.VideoCapture("C:\\Users\\Shiva Goud\\Desktop\\INFOSYS SPRING BOARD\\Face_Recognition\\Barak Obama\\Tools\\1000097197.mp4")
if not cam.isOpened():
    ...
    for face_encoding in face_encodings:
        ...
        if distance < confidence_threshold:
            ...
            for face_location in face_locations:
                ...
                cv.putText(frame, 'Barack Obama', (left, top - 10), cv.FONT_HERSHEY_SIMPLEX, 0.5,
            ...
            new_entry = pd.DataFrame({'Name': ['Barack Obama'], 'Date': [date_str], 'Time': [time_str]})
            df = pd.concat([df, new_entry], ignore_index=True)

    if not recognized:
        cv.putText(frame, 'Not Barack Obama', (30, 55), cv.FONT_HERSHEY_SIMPLEX, 1,
                   (255, 0, 0), 2, cv.LINE_AA)
...
if not df.empty:
    df.to_excel('Tools_attendance.xlsx', index=False)
