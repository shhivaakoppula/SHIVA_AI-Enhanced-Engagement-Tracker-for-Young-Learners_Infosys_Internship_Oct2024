...
if not os.path.exists("Shiva_screenshots(8)"):
    os.makedirs("Shiva_screenshots(8)")
...
try:
    while True:
        ...
        for (top, right, bottom, left), face_encoding in zip(face_locations, face_encodings):
            ...
            if distance < 0.6:
                ...
                screenshot_filename = f"Shiva_screenshots(8)/{name}_{now.strftime('%Y-%m-%d_%H-%M-%S')}.jpg"
                if attentive == 'Yes':
        ...
        if time.time() - start_time >= 30:
            ...
            if not df.empty:
                df.to_excel('Shiva_attendance_with_attention_and_average(8).xlsx', index=False)
                print("Attendance saved to 'Shiva_attendance_with_attention_and_average(8).xlsx'.")
    ...
    if not df.empty:
        ...
        df.to_excel('Shiva_attendance_with_attention_and_average(8).xlsx', index=False)
        print("Attendance with attentiveness and average score saved to 'Shiva_attendance_with_attention_and_average(8).xlsx'.")
