...
if not os.path.exists("Shiva_screenshots(4)"):
    os.makedirs("Shiva_screenshots(4)")

# Load the known image of Guru Teja
known_image = face_recognition.load_image_file("C:\\Users\\Shiva Goud\\Desktop\\INFOSYS SPRING BOARD\\Face_Recognition\\shiva.png")
...
try:
    while True:
        ...
        for face_encoding in face_encodings:
            ...
            if distance < confidence_threshold:
                ...
                if last_recognition_time is None or (now - last_recognition_time) >= entry_duration:
                    # Save screenshot
                    screenshot_filename = f"Shiva_screenshots(4)/Shiva_{now.strftime('%Y-%m-%d_%H-%M-%S')}.jpg"
        ...
        if (datetime.now() - last_save_time).total_seconds() >= 30:
            ...
            print("DataFrame saved to 'Shiva_attendance_with_screenshots(4).xlsx'")
