...
if not os.path.exists("Shiva_screenshots(7)"):
    os.makedirs("Shiva_screenshots(7)")
...
try:
    while True:
        ...
        if (datetime.now() - last_save_time).total_seconds() >= 30:
            df.to_excel('Shiva_attendance_with_attention_score(7).xlsx', index=False)
            print("DataFrame saved to 'Shiva_attendance_with_attention_score(7).xlsx'")
    ...
    if not df.empty:
        df.to_excel('Shiva_attendance_with_attention_score(7).xlsx', index=False)
        print("Final attendance with attentiveness saved to 'Shiva_attendance_with_attention_score(7).xlsx'.")
